...
public interface RecommendDao extends JpaRepository<Recommend, Long> {
    ...
    @Query(value = "select c.id,u.id 用户id,r.id 房间id,r.description,r.hall_name,r.image,r.price,r.room_name,r.status,r.del_flag from t_recommend c inner join t_room r on c.room_id = r.id inner join t_user u on c.user_id = u.id GROUP BY c.id ;" ,nativeQuery = true)
    List findRecommend();
}
